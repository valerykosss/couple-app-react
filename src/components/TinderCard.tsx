...
const { Meta } = Card;
// type CardWithNumericId = DateCardType & { numericId: number };
...
export const TinderCard = ({ card, cards, setCards, onSwipe }: TinderCardProps) => {
    ...
    const handleSwipeAction = (direction: 'left' | 'right') => {
        onSwipe({ id: card.originalId, direction }); // Используем originalId для Firebase
            ...
            actions={[
              <div style={iconsWrapperStyle}>
                <CloseOutlined
                  ...
                  onClick={() => isFront && onSwipe({ id: card.id, direction: 'left' })}
                />
                <HeartOutlined
                  style={heartOutlinedStyle}
                  onClick={() => isFront && onSwipe({ id: card.id, direction: 'right' })}
                />
              </div>
            ]}
          >
